...
public class RemoveCommand extends Command {
    ...
    public void execute(CommandSender sender, String[] args) {
        if (args.length != 3) {
            sender.sendMessage(StringUtils.color(IridiumMobCoins.getInstance().getConfiguration().prefix + " &7/mc remove <player> <amount>"));
            return;
        }
        int amount;
        try {
        ...
        if (offlinePlayer.hasPlayedBefore() || offlinePlayer.isOnline()) {
            ...
            sender.sendMessage(StringUtils.color(IridiumMobCoins.getInstance().getMessages().removePlayerMobCoins
                    .replace("%prefix%", IridiumMobCoins.getInstance().getConfiguration().prefix)
            ...
            if (player != null) {
                player.sendMessage(StringUtils.color(IridiumMobCoins.getInstance().getMessages().removedMobCoins
                        .replace("%prefix%", IridiumMobCoins.getInstance().getConfiguration().prefix)
                        .replace("%player%", user.getName())
                        .replace("%amount%", String.valueOf(amount))
                ));
